...
static void print_log_line(const char* appname, int level, char* msg_format, va_list msg_args);

static char* alloc_line(void) {
	char* ret_val = NULL;
	if ((ret_val = malloc(sizeof(char) * MAX_LOG_LEN)) == NULL) {
		print_err("LOGGING-INTERNAL", "Internal logging-error, couldn't allocate string for output");
	}
	return ret_val;
}
...
static void print_log_line(const char* appname, int level, char* msg_format, va_list msg_args) {
	...
	if ((line = alloc_line()) == NULL) {
		log_error(APPNAME_LOGGING, "Could not allocate memory for log line string!");
		return;
	}
	...
	if (errno != 0 && (level >= LEVEL_ERROR)) {
		...
		(void) fprintf(stderr, "%s %s: %s (errno=%d: %s)\n", level_str, appname, line, errno, stderrmsg);
	} else {
	...
	if (charsWritten > MAX_LOG_LEN) {
		log_warn(APPNAME_LOGGING, "WARN", "Log line was truncated by %d chars", (charsWritten - MAX_LOG_LEN));
	}
}
